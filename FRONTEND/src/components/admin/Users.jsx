...
function Users() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('https://localhost:44345/api/Users');
  ...
  const handleFilter = () => {
    ...
    const filtered = users.filter((user) => {
      return (
        ...
        (user.birth && user.birth.includes(searchTermLower)) ||
        (user.shift && user.shift.toLowerCase().includes(searchTermLower)) ||
  ...
  return (
    <div className="users flex flex-col w-[calc(100vw - 336px)] min-w-[1000px] max-w-[1200px] h-[100%] relative ">
      <div className="text-40px font-medium text-light-gray">Users</div>
      ...
      <div className="user-list mt-3 h-full min-h-[568px] w-[calc(100vw - 336px)] bg-black rounded-xl overflow-auto">
